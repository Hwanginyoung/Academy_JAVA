...
public class C02UpdDownCastingMain {
	...
	public static void main(String[] args) {
		...
		Func(new C(100,200));
		System.out.println();
		Func(new D(1000,2000));
	}
	
	public static void Func(A obj) { //A obj = new B(), A lbj = new C();
		...
		if(obj instanceof D) {
			...
			System.out.println("b : "obj.b); //확장된 멤버에 접근
			System.out.println("d : "obj.d); //확장된 멤버에 접근
		}
		else if(obj instanceof B) {
			B down = (B)obj;	//downcasting
			System.out.println("b : "obj.b); //다운캐스팅을 통한 확장된 멤버에 접근
		}
		else if(obj instanceof C) {
			C down = (C)obj;	//downcasting
			System.out.println("c : "obj.c); //확장된 멤버에0 접근
		}
	}
